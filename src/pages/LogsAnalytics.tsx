...
const INITIAL_LOGS = [
  ...
  { time: '10:42:30', level: 'INFO', source: 'AUTOSCALER', message: 'CPU utilization at 75%. Preparing to scale up pod-cluster-alpha.' },
  { time: '10:42:45', level: 'CRITICAL', source: 'IDS', message: 'SQL Injection attempt blocked on /api/v1/login. Source: 45.33.22.11' },
  { time: '10:43:00', level: 'INFO', source: 'AI_ENGINE', message: 'Autonomous mitigation applied. IP 45.33.22.11 added to global blocklist.' },
  { time: '10:43:10', level: 'INFO', source: 'SYSTEM', message: 'Health check passed for all core services.' },
];

const GENERATE_LOG = () => {
  const levels = ['INFO', 'INFO', 'WARN', 'CRITICAL'];
  const sources = ['AI_ENGINE', 'FIREWALL', 'AUTOSCALER', 'IDS', 'SYSTEM'];
  const messages = [
    ...
    'User session token expired and revoked.',
...
export function LogsAnalytics() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      // Add new log
      setLogs(prev => {
        const newLogs = [...prev, GENERATE_LOG()];
  ...
  return (
    <div className="flex flex-col h-full gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
          ...
          <div className="glass-panel rounded-2xl border border-white/10 p-5 flex flex-col flex-1 min-h-0">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-4 text-xs">
                <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-ai glow-blue" /> CPU Usage (%)</div>
          ...
          <div className="grid grid-cols-3 gap-4 h-32">
            <div className="glass-panel rounded-xl border border-white/10 p-4 flex flex-col justify-center">
              <div className="flex items-center gap-2 text-white/50 mb-2">
                <Cpu size={16} />
                <span className="text-xs uppercase tracking-wider">Avg CPU</span>
              </div>
        ...
        <div className="glass-panel rounded-2xl border border-white/10 flex flex-col min-h-0">
          <div className="bg-black/40 px-4 py-3 border-b border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-white/90">Live Event Stream</span>
            </div>
            <div className="flex items-center gap-1 bg-white/5 rounded-lg p-1">
              {['ALL', 'INFO', 'WARN', 'CRITICAL'].map(level => (
                <button
                  ...
                  className={cn(
                    "px-2 py-1 rounded text-[10px] font-bold tracking-wider transition-colors",
          ...
          <div className="flex-1 p-4 bg-[#0d1117] font-mono text-xs overflow-y-auto flex flex-col gap-2">
            {filteredLogs.map((log, i) => (
              ...
              >
                ...
                <span className="text-intel shrink-0 w-24 truncate">[{log.source}]</span>
